...
필요에 따라 이미지 최적화를 위한 기본 IO 구성 설정을 검토하고 업데이트합니다. 예를 들어 손실 형식에 대한 WebP 및 JPEG 품질 수준을 변경하거나 JPEG 이미지를 제공하는 형식을 _점진적_ 또는 _기준선_(으)로 변경할 수 있습니다. 또한 다음과 같이 보다 세분화된 이미지 최적화 기능을 위해 Fastly IO를 사용할 수 있습니다.
...
1. _이미지 최적화 기본 구성 옵션_ 페이지에서 Fastly IO 구성 설정을 검토하고 업데이트합니다.
   ...
   - **Auto WebP?**—이미지를 지원하는 브라우저에서 WebP 형식으로 변환하려면 기본 설정(`Yes`)을 그대로 둡니다. 설정을 **아니요**(으)로 변경하면 Fastly에서는 이미지를 WebP 형식으로 변환하는 대신 이미지 파일 형식을 사용합니다.
   ...
   - **업스케일링을 허용하시겠습니까?**—기본 설정(`No`)을 그대로 유지하거나 요청된 차원에 맞게 원본 소스 파일보다 큰 이미지를 반환하려면 `Yes`을(를) 선택하십시오.
