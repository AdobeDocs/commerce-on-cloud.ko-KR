---
title: 사용자 액세스 관리
description: Magento-cloud CLI 또는 Cloud Console을 사용하여 클라우드 인프라 프로젝트 및 환경에서 Adobe Commerce에 사용자를 추가하고 역할을 할당하는 방법을 알아봅니다.
role: Admin
feature: Cloud, Roles/Permissions
level: Beginner
short-description: Cloud Console 또는 CLI에서 사용자를 추가하고 프로젝트 및 환경 역할을 할당합니다.
...
>[!PREREQUISITES]
>
>- Adobe ID에 등록된 사용자. 사용자는 [Adobe 계정에 등록](https://account.adobe.com)한 다음 [클라우드 계정](https://console.adobecommerce.com)을 초기화해야 클라우드 프로젝트에 추가할 수 있습니다.
...
1. `magento-cloud` CLI를 사용하여 사용자를 추가하십시오.
   ...
   >[!IMPORTANT]
   >
   >사용자에게 Adobe ID이 있어야 합니다. 전제 조건을 참조하십시오.
...
1. 사용자를 추가한 후 모든 환경을 재배포하여 변경 사항을 적용합니다.

   재배포는 사용자가 SSH를 사용하여 환경에 액세스하거나 관리자 작업을 수행할 수 있도록 합니다.

사용자를 추가하면 Adobe은 클라우드 인프라 프로젝트에서 Adobe Commerce에 액세스하기 위한 지침과 함께 지정된 주소로 이메일을 전송합니다.

### 초대 상태

[!DNL Cloud Console]에서 관리자는 계정 초기화가 완료되기 전에 초대를 보낼 수 있습니다. 이 경우 액세스 목록에 사용자가 [!UICONTROL Invite pending] 상태로 표시됩니다. 온보딩이 완료될 때까지 액세스가 완전히 활성화되지 않습니다.

콘솔 및 사용자의 계정 상태에 따라 사용자는 다음 상태 중 하나에 표시될 수 있습니다.

- **[!UICONTROL Not invited]** — 프로젝트 액세스 레코드가 없습니다.
- **[!UICONTROL Invite pending]** — 초대가 전송되었지만 계정 초기화 또는 수락이 완료되지 않았습니다.
- **[!UICONTROL Active]** — 사용자가 온보딩을 완료하고 활성 프로젝트 액세스 권한이 있습니다.

>[!NOTE]
>
>[!DNL Cloud Console]은(는) [!DNL Legacy Cloud Console]&#x200B;(`https://<region-id>.magento.cloud/projects/<project_id>`)보다 더 명시적으로 초대 상태를 표시합니다. 사용자 또는 초대 항목이 표시된다고 해서 사용자가 모든 환경에 즉시 액세스할 수 있는 것은 아닙니다. SSH 키 설정 또는 기타 전달 단계가 계속 필요할 수 있습니다. [사용자 인증 요구 사항](#user-authentication-requirements)을 참조하세요.
...
- [작성(Android/iPhone)](https://authy.com/features/)
...
1. 인증자 애플리케이션에 클라우드 인프라의 Adobe Commerce 계정을 추가합니다.
   ...
   - **[!UICONTROL TFA set up - Application]** 페이지에서 **[!UICONTROL Application verification code]** 필드에 모바일 장치의 TFA 코드를 입력합니다.
...
1. **[!UICONTROL Create API token]**&#x200B;을(를) 클릭하고 이름을 입력합니다. 예를 들어, 컴퓨터 사용자 또는 API 토큰을 사용하는 자동화된 프로세스와 일치하는 이름을 지정합니다.
   ...
   ![API 토큰 이름 필드를 사용하는 클라우드 콘솔 API 토큰 탭](../../assets/api-token-name.png)
