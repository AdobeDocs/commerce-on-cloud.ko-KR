...
세 개의 서비스 노드는 각각 동일한 서비스 집합을 실행합니다. **OpenSearch** 또는 **Elasticsearch**, 데이터베이스는 **MariaDB**, 캐싱은 **Redis** 또는 **Valkey**. 서비스 계층이 용량에 근접하면 서버 크기(CPU 및 메모리)를 늘려 수직으로만 확장할 수 있습니다. 용량은 사용 가능한 가장 큰 노드 크기로 제한됩니다. 데이터베이스 클러스터는 고가용성을 위해 설계되었기 때문에 사용되는 기술을 사용하여 데이터베이스 노드를 신뢰할 수 있는 방식으로 확장할 수 없습니다.
...
요청 및 웹 트래픽을 처리하기 위한 웹 노드는 **php-fpm** 및 **NGINX**&#x200B;입니다. 파워와 메모리를 늘려 수직적으로 확장할 수 있을 뿐만 아니라, 웹 계층은 PHP 수준에서 제한될 때 기존 클러스터에 웹 서버를 추가하여 수평적으로 확장할 수 있습니다. 웹 노드의 자동 확장 방법을 알아보려면 [자동 확장](autoscaling.md)을 참조하십시오.

![웹 계층 크기 조정](../../assets/scaling-web.png)

이는 서비스 계층에서 제공하는 세로 크기 조절을 보완합니다. 서비스 계층이 증가하는 데이터베이스에 맞게 확장됨에 따라 웹 계층은 요청 및 트래픽 증가를 처리하도록 확장됩니다.

웹 노드 인스턴스 유형이 CPU 8개와 16Gb RAM이 있는 _C5.2xlarge_&#x200B;인 예를 생각해 보십시오. 사이트에 대한 요청 수가 크게 증가했습니다. php-fpm 프로세스의 증가를 처리하기 위해 C5.2xlarge 노드를 추가하거나 각 인스턴스 유형을 16개의 CPU과 32Gb RAM을 사용하는 _C5.4xlarge로 변경_&#x200B;할 수 있습니다. 노드를 추가하면 서지 용량이 부족해질 위험이 줄어듭니다.

## 프로젝트 구조

Scaled 아키텍처를 사용하는 Pro 프로젝트에는 6개의 노드가 있습니다.
...
로그 위치는 노드에 따라 약간 다릅니다. 예를 들어 **MySQL 오류 로그**(`/var/log/mysql/mysql-error.log`)는 서비스 노드에서는 사용할 수 있지만 웹 노드에서는 사용할 수 없습니다.
