...
[!DNL CrowdStrike Falcon], 경량의 차세대 EDR(Endpoint Detection and Response) 에이전트가 Adobe 내의 모든 끝점(서버 포함)에 설치됩니다. EDR 에이전트는 실시간 연속 모니터링 및 수집으로 Adobe 데이터 및 시스템을 보호하여 신속한 위협 식별 및 대응을 가능하게 합니다.
...
고객은 PCI 지침에 따라 릴리스 후 30일 이내에 이러한 패치가 사용자 정의 애플리케이션에 적용되었는지 확인해야 합니다. Adobe은 또한 가맹점이 사이트를 정기적으로 모니터링하고 알려진 보안 위험, 맬웨어 및 무단 액세스에 대한 업데이트를 받을 수 있는 [보안 검색 도구](https://experienceleague.adobe.com/en/docs/commerce-admin/systems/security/security-scan)를 제공합니다. 보안 검색 도구는 무료 서비스이며 모든 버전의 Adobe Commerce에서 실행할 수 있습니다.
...
백업은 지난 24시간 동안 매 시간마다 수행됩니다. 24시간 이후에는 AWS EBS 스냅샷 서비스를 사용하여 일정에 따라 백업이 유지됩니다. [보존 정책](pro-architecture.md#retention-policy)을 참조하세요.
