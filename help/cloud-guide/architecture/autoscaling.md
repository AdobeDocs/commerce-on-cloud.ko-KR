...
Auto scaling은 클라우드 인프라에 리소스를 자동으로 추가하거나 제거하여 최적의 성능과 합리적인 비용을 유지합니다. Adobe에서는 [!DNL Adobe Commerce on cloud infrastructure]개 프로젝트에 대해 두 가지 유형의 자동 확장을 제공합니다.

- [수평 자동 크기 조정](#horizontal-auto-scaling)(크기 조정된 아키텍처에만 사용 가능) — 크기 조정된 아키텍처 프로젝트의 웹 서버 노드를 추가하거나 제거합니다.
- [수직 자동 크기 조정](#vertical-auto-scaling)(표준 Pro 아키텍처 또는 크기 조정 아키텍처에 사용 가능) — 수요 변화에 맞게 기존 노드의 CPU 용량을 조정합니다.
...
현재 이 기능은 [크기 조정된 아키텍처](scaled-architecture.md)로 구성된 프로젝트에만 사용할 수 있습니다.

수평 자동 크기 조정 기능은 크기 조정된 아키텍처 프로젝트에 대한 웹 서버 노드를 추가하거나 제거합니다. 또는 [수직 자동 확장](#vertical-auto-scaling)은(는) 기존 노드의 CPU 용량을 조정하여 수요 변화에 맞게 조정합니다.
...
- **추가된 노드** — 모든 활성 웹 노드의 CPU/코어는 1분 동안 75% 용량이며 트래픽은 5분 연속 20% 증가하고 있습니다.
...
[!DNL Adobe Commerce on cloud infrastructure]은(는) 기존의 [수평 자동 크기 조정](#auto-scaling) 외에도 표준 Pro 아키텍처와 크기 조정된 아키텍처 프로젝트 모두에 대해 수직 자동 크기 조정을 제공합니다.

수직 Auto Scaling은 노드를 추가하거나 제거하는 대신 기존 노드의 CPU 용량 크기를 조정하여 수요 변화에 맞게 조정합니다. 이 기능은 수평 자동 크기 조절을 보완하여 크기 조정된 아키텍처 프로젝트에 대한 웹 서버 노드를 추가하거나 제거합니다.

- **노드가 추가됨**: 해당 사항 없음. 수직 자동 크기 조정은 새 노드를 추가하지 않고 기존 노드의 크기를 조정합니다.
- **노드 크기 조정**: 메모리 압력이 정의된 임계값을 초과할 때 노드 크기가 다음으로 큰 인스턴스 크기로 조정됩니다. 배율 이벤트당 하나의 크기 증가만 적용됩니다.
- **노드 축소**: 수요가 감소한 후 노드가 자동으로 축소됩니다. 각 프로젝트의 사용 패턴과 계약된 리소스 제한에 따라 최소 및 최대 크기가 설정되므로 불필요한 크기 조절의 위험이 줄어듭니다.

### 임계값 자동 확장

수직 자동 크기 조정 이벤트는 Linux의 메모리에 대한 PSI(Pressure Stall Information)를 사용하여 트리거되며, 이는 메모리 압력으로 인해 시스템이 정지되는 데 걸리는 시간을 측정합니다. Adobe은 프로젝트의 계약된 리소스 제한 및 사용 패턴을 기반으로 임계값을 설정합니다. 판매자는 현재 이를 구성할 수 없습니다.

### New Relic으로 임계값 모니터링

[!DNL New Relic] 서비스를 사용하여 인스턴스 크기 및 유형을 포함한 인프라 인스턴스 세부 사항을 모니터링할 수 있습니다. 수직 자동 크기 조정 이벤트가 인스턴스의 크기 또는 유형을 변경할 때마다 알림을 받도록 New Relic에서 경고를 구성합니다.

### 환경에 미치는 영향

수직 자동 크기 조정은 환경에 다음과 같은 영향을 줍니다.

- **가동 중지 시간**: 노드 크기를 조정할 때 예상되는 가동 중지 시간이 없습니다.
- **타이밍**: 노드의 크기를 조정하는 데 일반적으로 20~30분이 소요됩니다. 크기 조정이 진행되는 동안 노드가 일시적으로 로드 밸런서에서 제거됩니다.
