...
- 동일한 클라우드 영역 내의 동일한 클라우드 플랫폼(VPC 또는 Azure)에 있는 고객 Virtual Private Cloud(AWS)와 Adobe VPC 간의 보안 연결입니다.
...
1. 양방향 연결을 활성화하는 추가 단계:
   ...
   - **Customer**&#x200B;은(는) 고객 VPC의 끝점 서비스에 대한 Adobe 액세스를 가능하게 합니다. 이전에 **Endpoint Service 액세스 권한 부여** 전제 조건에 설명된 대로 Adobe 계정 주체가 `arn:aws:iam::402592597372:root`에 액세스할 수 있다고 가정합니다.
      ...
      - 고객 VPC에 대한 끝점 서비스 이름을 사용하는 Adobe을 제공합니다.
...
- VPC Adobe에서 고객 VPC 환경에서 사용할 수 있는 다른 엔드포인트 서비스로의 PrivateLink 연결을 추가하거나 제거합니다.
...
- Adobe이 VPC에서 호스팅되는 끝점 서비스에 연결할 수 있도록 하는 [인터페이스 끝점] (AWS) 또는 [개인 끝점] (Azure)
