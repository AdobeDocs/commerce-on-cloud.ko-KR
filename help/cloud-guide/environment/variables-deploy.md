...
- `false`—`Consumers`이(가) 큐에서 사용 가능한 메시지를 처리하고 TCP 연결을 닫고 종료합니다. `Consumers` 처리된 메시지 수가 `CRON_CONSUMERS_RUNNER` 배포 변수에 지정된 `max_messages` 값보다 작더라도 추가 메시지가 대기열에 들어갈 때까지 기다리지 마십시오.
